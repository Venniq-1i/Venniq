...
export default function AlertModeBEmail({ contract, deptName, leadName, matchedDept, responseToken, coAlertedDepts = [] }: Props) {
  ...
  return (
    ...
    >
      ...
      <Section style={{ background: brand.frost, borderLeft: `4px solid ${brand.navy}`, padding: '12px 16px', borderRadius: 4, marginBottom: 20 }}>
        <Text style={{ margin: 0, fontSize: 13, fontWeight: 600, color: brand.navy, fontFamily: F }}>{matchedDept.capability_match}</Text>
      ...
      {matchedDept.candidates.length > 0 && (
        <>
          <Text style={{ margin: '0 0 4px', fontSize: 14, fontWeight: 600, color: brand.navy, fontFamily: F }}>Recommended bid team candidates</Text>
